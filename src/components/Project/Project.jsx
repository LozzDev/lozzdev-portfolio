const Project = ({ backgroundColor, introduction, image, title, description, tools = [], githubLink, webLink, hasGithub, hasWeb, id}) => {
  return (
    <div id={`${id}`} className=" relative overflow-hidden rounded-2xl border border-white/15 bg-[#f2f2f20c] p-1.5 shadow-2xl lg:rounded-3xl lg:p-2 min-w-[340px] w-full">
      <div className="group relative flex  flex-col items-center justify-between rounded-xl lg:rounded-2xl bg-gradient-to-b from-black/40 to-transparent transition-all duration-300 overflow-hidden h-70 sm:h-80 md:h-90 xl:h-120">
        ...
        <img 
          src={`${image}`} 
          alt=""
          className="block w-full max-w-[85%] translate-y-6 -rotate-3 rounded-t-lg shadow-[0_35px_60px_-15px_rgba(0,0,0,1)] transition-all duration-300 will-change-transform group-hover:scale-[1.13] lg:group-hover:-rotate-3"
        />
      </div>
      ...
      <div className=" flex flex-col w-full gap-4">
        <div className="flex flex-row place-content-center">
          <a href={`${githubLink}`} target="_blank" rel="noopener noreferrer" className={` flex h-full  gap-2 rounded-xl bg-neutral-900 px-5 py-2 shadow-[inset_0_2px_10px_#ffffff1f] place-content-center ${hasGithub}`}>
          ...
          <a href={`${webLink}`} target="_blank" rel="noopener noreferrer" className= {`flex h-full gap-2 rounded-xl bg-neutral-900 px-5 py-2 shadow-[inset_0_2px_10px_#ffffff1f] place-content-center justify-self-center ${hasWeb}`}>
